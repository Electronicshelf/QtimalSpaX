...
def translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, filename):
    ...
    x_fake = nets.generator(x_src, s_src, masks=masks)
...
def debug_image(nets, args, inputs, step):
    ...
    x_ref, y_ref = inputs.x_src, inputs.y_src
...
def get_image_pairs(real_dir, distorted_dir, img_size):
    real_images = sorted([f for f in os.listdir(real_dir) if f != '.DS_Store'])
    distorted_images = sorted([f for f in os.listdir(distorted_dir) if f != '.DS_Store'])

    print(real_images)
    print(distorted_images)

    # Ensure both directories have the same number of files
    assert len(real_images) == len(distorted_images), "Mismatch in the number of real and distorted images."
    ...
    image_pairs = []
    for real_img, dist_img in zip(real_images, distorted_images):

        real_img_path = os.path.join(real_dir, real_img)
        dist_img_path = os.path.join(distorted_dir, dist_img)
        f_name = real_img

        # Open images, apply transformations, and convert to tensor
        real_image = Image.open(real_img_path).convert('RGB')  # Ensure RGB format
        dist_image = Image.open(dist_img_path).convert('RGB')

        real_tensor = transform(real_image).unsqueeze(0)  # Resize and convert to tensor
        dist_tensor = transform(dist_image).unsqueeze(0) # Resize and convert to tensor

        real_tensor = tensor2im(real_tensor.data)
        dist_tensor = tensor2im(dist_tensor.data)
        ...
        real_tensor= transform_x(real_tensor.astype(np.uint8)).unsqueeze(0)
        dist_tensor = transform_x(dist_tensor.astype(np.uint8)).unsqueeze(0)
        # print(real_tensor)
        # exit()
        image_pairs.append((real_tensor, dist_tensor, f_name))
...
def save_scores_to_file(score_hik, output_file, mode="hik"):
    """
    Save the similarity scores and filenames to a text file, line by line.
    Args:
        score_hik (list): A list of similarity scores.
        output_file (str): Path to the output text file.
    """
    output_dir = os.path.join(output_file, f'{mode}_scores.txt')
    try:
        with open(output_dir, 'w') as file:
            for score in score_hik:
                file.write(f"{score}\n")
        print(f"Scores successfully saved to {output_dir}")
